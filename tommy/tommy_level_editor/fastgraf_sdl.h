...
#define fg_boxx(x1,x2,y1,y2)  draw_boxx((x1-org_x)<<1, (y1-org_y)<<1, (x2-org_x)<<1, (y2-org_y)<<1, current_color)
#define fg_mousevis(v) if(v) SDL_ShowCursor(SDL_ENABLE); else SDL_ShowCursor(SDL_DISABLE)
#define fg_pan(xp,yp)  org_x = xp; org_y = yp; blit_scaled_X2_all(); show_screen()
#define fg_resize(w,h)  FG_SDL_init()
#define fg_reset()  FG_SDL_quit()
#define fg_print(fmt,n)  fg_sdl_print_scaled_X2(fmt, n);
#define fg_getkey(k,a)  while(!(*k = *a = get_input())) sleep()
#define fg_rect(x1,x2,y1,y2)  draw_rect((x1-org_x)<<1,(x2-org_x)<<1,(y1-org_y)<<1,(y2-org_y)<<1)
